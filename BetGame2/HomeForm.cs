...
namespace BetGame2
{
    public partial class HomeForm : Form
    {
        ...
        {
            // Cycle Details
            cycles[0] = new Cycle() { CycleName = "Cycle 1", RaceTrackLength = 1030, MyPictureBox = pictureCycle1 };
            cycles[1] = new Cycle() { CycleName = "Cycle 2", RaceTrackLength = 1030, MyPictureBox = pictureCycle2 };
            cycles[2] = new Cycle() { CycleName = "Cycle 3", RaceTrackLength = 1030, MyPictureBox = pictureCycle3 };
            cycles[3] = new Cycle() { CycleName = "Cycle 4", RaceTrackLength = 1030, MyPictureBox = pictureCycle4 };
                    ...
                    { 
                        ...
                        if (punter.MyRadioButton.Checked)
                        {
                            if (punter.MyBet == null)
                            {
                                ...
                                {
                                    if (pun.MyBet != null && pun.MyBet.Cycle == cycles[number - 1])
                                    {
                                        alreadyPlaced = true;
                                        break;
                                    }
                                ...
                                {
                                    punter.MyBet = new Bet() { Amount = amount, Cycle = cycles[number - 1] };
                                }
        ...
        private void Cycling_Tick(object sender, EventArgs e)
        {
            if(sender is Timer)
            {
                ...
                if( index != -1 )
                {
                    PictureBox pbox = cycles[index].MyPictureBox;
                    if (pbox.Location.X + pbox.Width > cycles[index].RaceTrackLength)
                    {  
                        if (winnerCycle == null)
                        {
                            winnerCycle = cycles[index];
                        }
                        timer1.Stop();
                        timer2.Stop();
                        timer3.Stop();
            ...
            if( winnerCycle != null)
            {
                MessageBox.Show("Hurray!!! " + winnerCycle.CycleName + " is Won...");
                SetupBetDetails();
                foreach (Punter punter in punters)
                {
                    if (punter.MyBet != null)
